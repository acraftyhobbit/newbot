...
def validate(sender_id, message_text, attachment_type, postback, quick_reply):
    ...
    if quick_reply and quick_reply.lower() in ['add_pattern', 'select_pattern']:
        return True, dict()
            ...
            'add' in message_text.lower() or 'select' in message_text.lower()) and 'pattern' in message_text.lower():
        return True, dict()
    else:
        return False, dict(message_text="I'm sorry, did you want to add a new pattern or select an existing one?",
                           quick_replies=[
                               {
                                   "content_type": "text",
                                   "title": "New Pattern",
